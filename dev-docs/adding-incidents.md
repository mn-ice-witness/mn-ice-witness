...
**After writing the incident file, do a complete second pass to verify everything.** This is not optional — every new incident goes through two full passes.

### Why Two Passes

The first pass focuses on research and writing. Details get missed, sources get cited without reading, and assumptions creep in. The second pass catches these errors before they go live.

### What to Check on the Second Pass

**Re-read every source.** Go back to each source URL and verify:
- The source actually covers THIS incident (not a similar one)
- Names, dates, and locations match what you wrote
- Quotes are accurate and attributed correctly
- You haven't added details that aren't in the sources

**Re-fetch any sources you couldn't access.** If WebFetch failed on any URL during the first pass:
1. Print the URL

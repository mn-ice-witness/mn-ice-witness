...
- `docs/incidents/2025-12/2025-12-early-lomabonita-jose.md`

**IMPORTANT:** All incident files MUST be in `docs/incidents/`. The website serves directly from `docs/`.

## Frontmatter Fields

### ⚠️ CRITICAL: Date Format Breaks GitHub Pages Build

**The `date` field MUST be a valid date in `YYYY-MM-DD` format.**
...
```yaml
---
date: YYYY-MM-DD              # Required. MUST be valid date (e.g., 2026-01-15). NO text like "early", "mid", "late"!
...
3. **Be accessible** - The link must be a valid URL to the content. Paywalled sources from major outlets (Star Tribune, Pioneer Press, etc.) are valid — do NOT mark them as "(paywalled)" in the source list.

...
| `loadIncidents()` | Fetch category JSON files in parallel, deduplicate |
...
**Note:** Dual-category incidents appear in both relevant files. The frontend fetches all 5 files in parallel and deduplicates by filePath.

---

### `docs/data/search-index.md`

**Purpose:** Lightweight search index for LLM consumption during daily incident searches.

**Auto-generated by:** `scripts/generate_summary.py`

**Format:** One line per incident: `date | city | category | title`

**When to use:** Read this file FIRST when searching for new incidents to quickly check if something already exists.
...
4. Outputs 5 category JSON files to `docs/data/incidents-summary-{category}.json`
5. Outputs `docs/data/search-index.md` for LLM searching
6. Dual-category incidents are included in both relevant category files

...
- **Data Loading**: `loadIncidents()` fetches category JSON files in parallel
...
- `incidents-summary-background.json`
...
**Note:** Dual-category incidents appear in both relevant files. The frontend fetches all 6 files in parallel and deduplicates by filePath.

---

### `docs/data/search-index.md`

**Purpose:** Comprehensive search index for LLM consumption during daily incident searches.
...
**When to use:** Read this file FIRST when searching for new incidents. It replaces the need to separately read not_use.md.
...
### `scripts/generate_summary.py` (~300 lines)

**Purpose:** Generate category-based incident summary files and search index.
...
4. Outputs 6 category JSON files to `docs/data/incidents-summary-{category}.json`
5. Outputs `docs/data/search-index.md` for LLM searching
6. Outputs `docs/sitemap.xml` with all incident URLs
7. Appends new media slugs to `docs/data/media-order.md`
8. Dual-category incidents are included in both relevant category files

**All outputs are auto-staged by the pre-commit hook.** Never edit these files directly.

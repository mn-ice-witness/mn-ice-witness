...
def find_existing_og_images(video_output_path: Path) -> list[Path]:
    """Find all existing OG images for a video (any timestamp/mtime)."""
    stem = video_output_path.stem
    pattern = f"{stem}-og-*.jpg"
    return list(video_output_path.parent.glob(pattern))


def cleanup_wrong_og_images(video_output_path: Path, correct_og_path: Path) -> int:
    """Delete OG images with wrong timestamps. Returns count of deleted files."""
    deleted = 0
    existing = find_existing_og_images(video_output_path)
    ...
    old_style = video_output_path.parent / f"{video_output_path.stem}-og.jpg"
    if old_style.exists():
...
def main():
    ...
    for video_path in sorted(video_outputs):
        ...
        if custom_og_source:
            ...
            og_cleaned += cleanup_wrong_og_images(video_path, og_path)

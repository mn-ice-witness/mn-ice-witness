/**
 * Cloudflare Pages Function for /corrections
 *
 * Intercepts the corrections page URL, injects Open Graph meta tags for social media sharing,
...
export async function onRequest(context) {
  ...
  // Build OG meta tags for corrections page
  const ogTags = buildCorrectionsOgTags(baseUrl.origin);
...
function buildCorrectionsOgTags(origin) {
  const title = 'Corrections | MN ICE Witness';
  const description = 'Incidents corrected after significant factual errors were identified.';
  const url = `${origin}/corrections`;

...
const Lightbox = {
    ...
    /**
     * Open removed incidents page
     */
    openRemoved() {
        this.element.setAttribute('aria-hidden', 'false');
        document.body.style.overflow = 'hidden';

        this.currentSlug = 'removed';
        const targetPath = Router.buildUrl('removed');

        if (window.location.pathname !== targetPath) {
            history.pushState({ lightbox: true, slug: 'removed' }, '', targetPath);
            this.openedViaPushState = true;
        } else {
            history.replaceState({ lightbox: true, slug: 'removed' }, '', targetPath);
            this.openedViaPushState = false;
        }

        const removedIncidents = App.getRemovedIncidents();
        this.bodyElement.innerHTML = this.renderRemovedContent(removedIncidents);
        this.bodyElement.querySelector('.share-btn')?.addEventListener('click', () => this.copyShareLink());
        this.setupRemovedLinks();
    },

    /**
     * Render removed page content
     */
    renderRemovedContent(incidents) {
        const shareButton = LightboxContent.renderShareButton();
    ...
    /**
     * Setup click handlers for removed incident links
     */
    setupRemovedLinks() {
        this.bodyElement.querySelectorAll('.removed-link').forEach(link => {
            link.addEventListener('click', (e) => {
                e.preventDefault();
                const slug = link.dataset.slug;
                this.openIncidentBySlug(slug);
